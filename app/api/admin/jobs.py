...
async def job_extract(ticker: str) -> int:
    """Extract content for a single ticker and return update count."""
    started = datetime.utcnow().isoformat() + "Z"
    ...
    try:
        async with aiosqlite.connect(db_path) as db:
            ...
            async with db.execute(q, (ticker,)) as cur:
                rows = await cur.fetchall()
    except Exception as e:
        log.exception("job_extract: DB load failed for ticker=%s", ticker)
        insert_run("extract", ticker, 0, f"db_load_err={e}", started_at=started)
        return 0
    
    if not rows:
        insert_run("extract", ticker, 1, "no_candidates", started_at=started)
        return 0
    ...
    for r in (rows or [])[:max_tries]:
        ...
        try:
            ...
            log.exception("job_extract: failed for url=%s", url)
            continue
    
    log.info("job_extract: ticker=%s tried=%d updated=%d", ticker, tried, updated)
    note = f"tried={tried} updated={updated}"
    insert_run("extract", ticker, 1, note, started_at=started)
    return updated
